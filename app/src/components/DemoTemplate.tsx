...
import { PageTemplateProps, usePageContext } from './PageTemplate';
 ...
 */
interface DemoTemplateProps extends PageTemplateProps {
 ...
 */
export const DemoTemplate: React.FC<DemoTemplateProps> = ({ children, page = 5, withoutProgressbar = false }) => {
  const { t, styles, navigate } = usePageContext(defaultStyles); // get translations, styles, and navigation
  const { getDemoRouteCount } = useTheme(); // get total number of test pages
  ...
  return (
    <div style={styles.demoFrame}>
      ...
      <div style={styles.titleBar}>
        ...
        {!withoutProgressbar && <ProgressBar value={page} max={getDemoRouteCount()} />}
